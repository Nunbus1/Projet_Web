...
document.addEventListener("DOMContentLoaded", function () {
  ...
  async function fetchArbres(page = 1) {
    try {
      ajaxRequest('GET', `../../back/php/map.php?action=getArbres&page=${page}`, function(data) {
  ...
  function displayTable(arbres) {
    ...
    arbres.forEach((arbre) => {
      ...
      row.innerHTML = `
        <td><input type="radio" name="selectedArbre" class="arbre-checkbox" data-id="${
          arbre.id_arbre
        }"></td>
        <td>${arbre.nom}</td>
  ...
  document
    .getElementById("predict-age-button")
    ...
    .addEventListener("click", function () {
      ...
      if (selectedArbre) {
        ...
        ajaxRequest(
          ...
          `id=${arbreId}&action=deracinement`
        );
      } else {
        alert("Veuillez sélectionner un arbre.");
      }
